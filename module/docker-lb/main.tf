...
resource "aws_lb_target_group" "docker_lb_TG" {
  name     = var.lb_TG_name
...
resource "aws_lb_listener" "docker_lb_listener" {
  load_balancer_arn = aws_lb.docker_lb.arn
  port              = var.http_port
  protocol          = "HTTP"
  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.docker_lb_TG.arn
  }
}

#creating Load balancer Target Group Attachment
resource "aws_lb_target_group_attachment" "docker_lb_tg_att" {
  target_group_arn = aws_lb_target_group.docker_lb_TG.arn
